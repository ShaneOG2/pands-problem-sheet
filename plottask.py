...
f_ypoints = xpoints
g_ypoints = xpoints**2 # Calculates x to the power of 2
h_ypoints = xpoints**3

# Plots each of the three fuctions, gives them each a colour and label
plt.plot(xpoints,f_ypoints,color="r",label="f(x)=x")
plt.plot(xpoints,g_ypoints,color="g",label="g(x)=$\mathregular{x^2}$") # Writes x to the power of 2 nicely on output
plt.plot(xpoints,h_ypoints,color="b",label="h(x)=$\mathregular{x^3}$")
